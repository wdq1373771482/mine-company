...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        function refreshDate(){
            ...
            monthScroll.scrollTo(0, (initM+11)*40, 100, true);
            dayScroll.scrollTo(0, (initD+opts.endday-1)*40, 100, true); 
        }
	...
	function resetIndex(){
        ...
        function bindButton(){
            resetIndex();
            $(".dh").unbind('click').click(function () {	
            ...
            var strM = $("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1)
              ...
              monthScroll = new iScroll("monthwrapper",{snap:"li",vScrollbar:false,
                 onScrollEnd:function (){
                  ...
                  if(indexM<opts.endmonth){
                    indexM=Math.floor(indexM)+opts.endmonth
                  }
                  if(indexM>(opts.endmonth*2)){
                    indexM=Math.floor(indexM)-opts.endmonth
                  }  
                    ...
                    dayScroll.refresh();
                    $(this.scroller).css('transform','translate3d(0px,'+(-(indexM-1)*40)+'px, 0px) scale(1)')
              ...
              dayScroll = new iScroll("daywrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function () {
                      ...
                      indexD = (this.y/40)*(-1)+1;
                       if(indexD<opts.endday){
                        indexD=Math.floor(indexD)+opts.endday
                      }
                      if(indexD>(opts.endday*2)){
                        indexD=Math.floor(indexD)-opts.endday
                      }  
                    $(this.scroller).css('transform','translate3d(0px,'+(-(indexD-1)*40)+'px, 0px) scale(1)')
                ...
                '<div id="datePage" class="page">'+
                    '<section>'+
                       /*  '<div id="datetitle"><h1>请选择日期</h1></div>'+ */
                        ...
                        '<div id="datescroll"><div class="li_t">&nbsp;&nbsp;</div>'+
                            '<div id="yearwrapper">'+
                                '<ul class="rq"></ul>'+
                            '</div><div class="li_d">&nbsp;&nbsp;</div>'+
                            '<div id="monthwrapper">'+
                                '<ul></ul>'+
                            '</div>'+
                            '<div id="daywrapper">'+
